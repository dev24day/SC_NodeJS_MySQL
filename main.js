...
var app = http.createServer(function(request, response) {
	...
	if (pathname === '/') {
		...
		db.query('SELECT * FROM topic', function(error, topics) {
			if (error) throw error;
			var filteredId = path.parse(queryData.id).base;
			db.query(`SELECT * FROM topic WHERE id=?`, [filteredId], function(
				error2,
				result,
				fields
			) {
				...
				db.query('SELECT * FROM author', function(error3, authors) {
					...
					var html = template.HTML(
						sanitizedTitle,
						'',
						`<form action="/update_process" method="post"><input type="hidden" name="id" value="${filteredId}"><p><input type="text" name="title" placeholder="title" value="${sanitizedTitle}"></p><p><textarea name="description" placeholder="description">${sanitizedDesc}</textarea></p><p>${template.author_combobox(
							authors,
							result[0].author_id
