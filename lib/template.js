...
exports.authorTable = function(authors, fields) {
  ...
  authors.forEach(author => {
	...
	table += `<td><a href="/author_update?author_id=${author.id}"><button type="submit">⟳</button></a></td>`;
	table += `<td><form action="/author_delete_process" method="post"><input type="hidden" name="id" value="${author.id}"><button type="submit">⌧</button></form></td>`;
    table += "</tr>";
  });
  table += "</table>";
  return table;
